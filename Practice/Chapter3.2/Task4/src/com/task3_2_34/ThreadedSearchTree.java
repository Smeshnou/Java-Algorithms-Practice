...
public class ThreadedSearchTree<K extends Comparable<K>, V> {
    private Node root;  
    ...
    private Node put(Node node, K key, V val){
        ...
        Node succeeding = min(node.right);
        if(succeeding != null){
            node.succ = succeeding;
            succeeding.pred = node;
        }
        else if(node == max(root)){
            node.succ = null;
        }

        Node previous = max(node.left);
        if(previous != null){
            node.pred = previous;
            previous.succ = node;
        }
        else if(node == min(root)){
            node.pred = null;
        }
        return node;
    }
    ...
    public K next(K key){
        if(get(root, key) != null && get(root, key).succ != null){
            return get(root, key).succ.key;
        }
        else{
            return null;
        }
    ...
    public K prev(K key){
        if(get(root, key) != null && get(root, key).pred != null){
            return get(root, key).pred.key;
        }
        else{
            return null;
        }
    ...
    public void print(){
        ...
        for(int i = 0; i < size(root); i++){
            logger.info(node.key.toString());
            node = node.succ;
        }
    }

    private Logger logger = Logger.getLogger(ThreadedSearchTree.class.getName());
    public static void main(String[] args){
        ThreadedSearchTree<String, Integer> tree = new ThreadedSearchTree<>();
